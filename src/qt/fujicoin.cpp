...
static bool InitSettings()
{
    ...
    if (!gArgs.ReadSettingsFile(&errors)) {
        std::string error = QT_TRANSLATE_NOOP("fujicoin-core", "Settings file could not be read");
        std::string error_translated = QCoreApplication::translate("fujicoin-core", error.c_str()).toStdString();
    ...
    if (!gArgs.WriteSettingsFile(&errors)) {
        std::string error = QT_TRANSLATE_NOOP("fujicoin-core", "Settings file could not be written");
        std::string error_translated = QCoreApplication::translate("fujicoin-core", error.c_str()).toStdString();
        InitError(Untranslated(strprintf("%s:\n%s\n", error, MakeUnorderedList(errors))));
